select timestamp_date as date,
    ...
    post_trigger_date,
    post_expiry_date,
    sum(event_impression_flag) as impresssions,
    sum(event_view_flag) as views,
    sum(event_like_flag) as likes,
    count(
        distinct case
            when event_impression_flag = 1 then user_id 
        end
    ) as impressioned_cjs,
    count(
        distinct case
            when event_view_flag = 1 then user_id
        end
    ) as viewed_cjs,
    count(
        distinct case
            when event_like_flag = 1 then user_id
        end
    ) as liked_cjs,
    from (
        select distinct PARAM_CJ_EMP_ID,
            ...
            config.cj_view_flag,
            config.cj_like_flag,
            config.event_impression_flag,
            config.event_view_flag,
            config.event_like_flag,            
        from "GA_DB"."INS_APP_GA4"."EVENT" e1 --     left join (select distinct param_feed_id,
            ...
            left join (select * from PC_STITCH_DB.FIVETRAN1_BI.feed_24_config limit 19) config on e1.name=config.name
            left join (
                select cj._id,
